...
<h3 align="center"> <a href="#client"></a> Вариант 15. Гостиница
  ...
  - поиск клиентов с максимальным сроком проживания </h3>
...
<ol>
  <li>Выборка из одной таблицы
    <ol>
      <li>Выбрать из произвольной таблицы данные и отсортировать их по двум произвольным признакам (разные направления сортировки).</li>
      <li>Выбрать из произвольной таблицы записи, удовлетворяющие условию отбора (WHERE). Привести 2–3 запроса.</li>
      <li>Примеры 2–3 запросов с агрегатными функциями (COUNT, MAX, SUM и др.) с группировкой и без.</li>
      <li>Примеры подведения итогов с использованием GROUP BY [ALL] [CUBE | ROLLUP] (2–3 запроса). В ROLLUP и CUBE использовать не менее 2 столбцов.</li>
      <li>Выборка объектов, в названиях которых нет заданной последовательности букв (LIKE).</li>
    </ol>
  </li>
  <li>Выборка из нескольких таблиц
    <ol>
      <li>Вывод информации из дочерней таблицы с заменой внешних ключей на символьные значения из родительских таблиц (2–3 запроса через WHERE).</li>
      <li>Реализация п. 2.1 через INNER JOIN.</li>
      <li>Левое внешнее соединение (LEFT JOIN, 2–3 запроса).</li>
      <li>Правое внешнее соединение (RIGHT JOIN, 2–3 запроса).</li>
      <li>Примеры 2–3 запросов с агрегатными функциями и группировкой.</li>
      <li>Примеры 2–3 запросов с группировкой и условием HAVING.</li>
      <li>Примеры 3–4 вложенных запросов (IN, EXISTS).</li>
    </ol>
  </li>
  <li>Представления
    <ol>
      <li>Создать два представления (VIEW) на основе запросов из п. 2.</li>
      <li>Примеры 2–3 запросов с использованием общетабличных выражений (CTE).</li>
    </ol>
  </li>
  <li>Функции ранжирования
    <ol>
      <li>Примеры 3–4 запросов с использованием ROW_NUMBER, RANK, DENSE_RANK (с PARTITION BY и без).</li>
    </ol>
  </li>
  <li>Объединение, пересечение, разность
    <ol>
      <li>Примеры 3–4 запросов с использованием UNION / UNION ALL, EXCEPT, INTERSECT (в одном из запросов сортировка).</li>
    </ol>
  </li>
  <li>Использование CASE, PIVOT и UNPIVOT
    <ol>
      <li>Примеры получения сводных таблиц с CASE.</li>
      <li>Примеры получения сводных таблиц с PIVOT и UNPIVOT.</li>
    </ol>
  </li>
</ol>

<p><b>Обязательными к выполнению</b> являются запросы, приведенные ниже (смотри свой вариант).  
Отчёт оформляется в виде документа <i>Фамилия_Группа.docx</i>, где по каждому заданию нужно указать условие, SQL-запрос и скриншот результата.</p>
...
<ol>
  ...
  <li>Найти клиентов, выехавших раньше срока, указанного в договоре.</li>
  <li>Найти номера, которые ни разу не сдавались с начала текущего года.</li>
  <li>Найти клиентов, забронировавших номер, но не въехавших в него.</li>
  <li>Среди наиболее частых клиентов найти тех, у кого максимальный срок проживания.</li>
</ol>
