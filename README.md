...
<div>
  <p><b>Задание:</b> разработать и реализовать в СУБД MSSQL хранимые процедуры, функции и триггеры в соответствии с заданием варианта.</p>

  <p><b>Необходимо создать:</b></p>

  <h4>4 хранимые процедуры</h4>
  <ol type="a">
    <li><b>Процедура без параметров</b>, формирующая список номеров (номер, комфортность, цена), которые никогда не бронировались и не заселялись.</li>
    <li><b>Процедура с параметрами</b> (комфортность, дата заезда, срок поселения), формирующая список подходящих номеров с учётом того, что они на этот срок не должны быть заняты или забронированы.</li>
    <li><b>Процедура с входным параметром</b> — ФИО клиента, и <b>выходным параметром</b> — дата его последнего заселения.</li>
    <li><b>Процедура, вызывающая вложенную процедуру</b>, которая подсчитывает среднюю посещаемость клиентами гостиницы (сколько раз в среднем каждый клиент пользовался услугами). Главная процедура выводит список клиентов, число посещений которых больше среднего.</li>
  </ol>

  <h4>3 пользовательские функции</h4>
  <ol type="a">
    <li><b>Скалярная функция</b>, возвращающая ФИО клиента, прожившего в гостинице наибольшее количество дней в сумме за все посещения. Если таких несколько, вернуть клиента с наименьшим ID.</li>
    <li><b>Inline-функция</b>, возвращающая список номеров, забронированных на данный момент, но реально не занятых (клиенты нас «обманули»).</li>
    <li><b>Multi-statement-функция</b>, возвращающая список забронированных номеров, в которые клиенты так и не въехали за весь период брони.</li>
  </ol>

  <h4>3 триггера</h4>
  <ol type="a">
    <li><b>Триггер на добавление брони</b> — если дата заезда больше текущей на 6 месяцев, цена на такую бронь увеличивается на 10%.</li>
    <li><b>Последующий триггер</b> на изменение фактической даты освобождения — если клиент прожил в гостинице дольше, чем указано в договоре, пересчитать общую стоимость проживания.</li>
    <li><b>Замещающий триггер</b> на удаление номера — если номер в данный момент занят, удаление запрещено и выводится сообщение об ошибке; если не занят — удалить номер и всю связанную информацию о его сдаче и бронировании.</li>
  </ol>

  <p><b>Примечание:</b> обязательно предусмотреть обработку нескольких записей. При необходимости использовать <b>курсор</b>.</p>

  <p>Отчет по лабораторной работе предоставляется в виде документа <i>(Фамилия_Группа.docx)</i>, в котором необходимо привести:
  текст созданных объектов, скриншоты их выполнения и результаты работы.</p>

  <a href="https://github.com/shidendi/databases_Shipitsyn_PMI-32_15variant/raw/main/Doc/Шипицын_ПМИ-32БО_Lab4.docx" target="_blank">Лабораторная №4</a>
</div>
